...
class HistoryEmbedding(nn.Module):
    ...
    def _projection_generator(self, anchor_idx, target_idx, anchor):
        ...
        interaction_vec = self.interactions[anchor][anchor_idx, 2:].clone().float()

        # masking target
        VALID = target_idx > 1
        batch_idx = torch.arange(anchor_idx.size(0), device=anchor_idx.device)
        interaction_vec[batch_idx[VALID], target_idx[VALID]-2] = 0.0
